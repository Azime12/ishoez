    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (data.products && data.products.length > 0) {
          // Filter out products with no price or price = 0
          const validProducts = data.products.filter(
            (p) => p.price && parseFloat(p.price) > 0
          );
          if (validProducts.length > 0) {
            setProducts(validProducts);
          } else {
            setError("No products found with valid price.");
          }
        } else {
            ...
            >
              ...
              <div className="flex flex-col flex-grow p-4">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-3 py-2 mt-auto text-center text-white rounded bg-emerald-500 hover:bg-emerald-600"
                >
                  Check Price
                </a>
              </div>
